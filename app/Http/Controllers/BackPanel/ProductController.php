        ...
        try {
            ...
            foreach ($data as $row) {
                ...
                if (!empty($post['type']) && $post['type'] === 'trashed') {
                    ...
                    $array[$i]['status'] = '<form action="' . route('admin.product.toggle-active', $row->id) . '" method="POST" class="d-inline js-confirm-submit" data-confirm-title="' . ($row->is_active ? 'Deactivate this product?' : 'Activate this product?') . '" data-confirm-text="This will change the product availability in the backend." data-confirm-button="Yes, update status">' . csrf_field() . '<button type="submit" class="btn btn-sm ' . ($row->is_active ? 'btn-outline-success' : 'btn-outline-danger') . ' table-action-btn status-toggle-btn" title="' . ($row->is_active ? 'Deactivate Product' : 'Activate Product') . '" aria-label="' . ($row->is_active ? 'Deactivate Product' : 'Activate Product') . '"><i class="fa-solid ' . ($row->is_active ? 'fa-toggle-on' : 'fa-toggle-off') . '"></i></button></form>';
                }
